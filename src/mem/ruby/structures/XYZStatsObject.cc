...
void XYZStatsObject::regStats() {
    ...
    total_puts.name(name() + ".total_puts").desc("Total number of PutS"); // may be incorrect
    total_putm.name(name() + ".total_putm").desc("Total number of PutM"); // may be incorrect
    ...
    total_puts_as_wt.name(name() + ".total_puts_as_wt").desc("Total number of PutS as write through"); // may be incorrect
    total_putm_as_wt.name(name() + ".total_putm_as_wt").desc("Total number of PutM as write through"); // may be incorrect
    wt_putRatio.name(name() + ".wt_putRatio").desc("WT / Put"); // may be incorrect
    wt_putsRatio.name(name() + ".wt_putsRatio").desc("PutS WT / PutS"); // may be incorrect
    wt_putmRatio.name(name() + ".wt_putmRatio").desc("PutM WT / PutM"); // may be incorrect
